...
void IO::writeText(int fd, unsigned int account, const std::string& text, unsigned int protocolID) {
    /* send head data */
    Encoder encoder;
...
void IO::writeImg(int fd, unsigned int account, const std::string& imgpath, unsigned int protocolID) {
    ...
    /* send head data */
    Encoder encoder;
    std::string headstr = encoder.encode(protocolID, account, IMG, imageSize);
    send(fd, headstr.data(), headstr.length(), MSG_NOSIGNAL);
